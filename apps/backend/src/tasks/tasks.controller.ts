...
export class TasksController {
    ...
    async getPublicNearbyTasks(
        ...
        return this.tasksService.getPublicNearbyTasks(
            parseFloat(lat),
            parseFloat(lng),
    ...
    @Post(':id/rate')
    @HttpCode(HttpStatus.OK)
    async rateTaskPublicly(
        @Param('id') id: string,
        @Body() body: { score: number },
    ) {
        if (!body.score || body.score < 1 || body.score > 5) {
            throw new BadRequestException('Valid score between 1 and 5 is required');
        }
        return this.tasksService.rateTaskPublicly(id, body.score);
    }
