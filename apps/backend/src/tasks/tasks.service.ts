...
export class TasksService {
    ...
    async createTask(clientId: string, dto: any, beforeImages?: Express.Multer.File[]) {
        ...
        if (dto.requiresLocation && !dto.locationName?.trim()) {
        ...
        const acceptanceTime = dto.acceptanceTime ? new Date(dto.acceptanceTime) : undefined;
        ...
        await this.redis.delPattern('tasks:marketplace:*');
    ...
    async getOpenTasks(filters?: { category?: string; minBudget?: number; maxBudget?: number }) {
        ...
        const cacheKey = `tasks:marketplace:${JSON.stringify(filters || {})}`;
        ...
        // Cache for 2 minutes
        await this.redis.set(cacheKey, tasks, 120);
    ...
    // PUBLIC: Get nearby tasks
    async getPublicNearbyTasks(lat: number, lng: number, radiusMeters: number) {
        const tasks = await this.prisma.task.findMany({
            where: {
                workerId: { not: null },
        ...
        // Filter by radius using calculateDistance
        const nearbyTasks = tasks.filter(task => {
            ...
            const distance = this.calculateDistance(lat, lng, destLat, destLng);
    ...
    // PUBLIC: Rate a task
    async rateTaskPublicly(taskId: string, score: number) {
        const task = await this.getTaskById(taskId);
        
        if (!task.workerId) {
            throw new BadRequestException('Task does not have an assigned worker');
        }

        const currentTaskRating = task.publicRating || 0;
        const currentTaskRatingCount = task.publicRatingCount || 0;
        
        const newTaskRatingCount = currentTaskRatingCount + 1;
        const newTaskRating = ((currentTaskRating * currentTaskRatingCount) + score) / newTaskRatingCount;

        await this.prisma.task.update({
            where: { id: taskId },
            data: {
                publicRating: newTaskRating,
                publicRatingCount: newTaskRatingCount,
            }
        });

        // Update worker's overall rating
        const dbWorker = await this.prisma.user.findUnique({ where: { id: task.workerId } });
        if (!dbWorker) throw new NotFoundException('Worker not found in DB');

        const newWorkerRatingCount = dbWorker.ratingCount + 1;
        const newWorkerRating = ((dbWorker.rating * dbWorker.ratingCount) + score) / newWorkerRatingCount;

        await this.prisma.user.update({
            where: { id: task.workerId },
            data: {
                rating: newWorkerRating,
                ratingCount: newWorkerRatingCount,
            }
        });

        // Invalidate marketplace cache so updates are reflected
        await this.redis.delPattern('tasks:marketplace:*');

        return { success: true, newWorkerRating, newTaskRating };
    }
    ...
    async getMyTasks(userId: string, role: 'client' | 'worker') {
        // Generate cache key
        const cacheKey = `tasks:my:${userId}:${role}`;
    ...
    async updateTask(taskId: string, clientId: string, dto: any, beforeImages?: Express.Multer.File[]) {
        ...
        // Invalidate caches
        await this.redis.delPattern('tasks:marketplace:*');
        await this.redis.del(`tasks:my:${clientId}:client`);
