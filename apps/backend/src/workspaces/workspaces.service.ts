...
export class WorkspacesService {
    ...
    async createWorkspace(userId: string, name: string, description?: string) {
        ...
        // Invalidate cache
        await this.redis.del(`workspaces:user:${userId}`);
    ...
    async getUserWorkspaces(userId: string) {
        ...
        // Cache for 3 minutes
        await this.redis.set(cacheKey, workspaces, 180);
    ...
    async updateWorkspace(workspaceId: string, userId: string, data: { name?: string; description?: string }) {
        ...
        for (const member of workspace.members) {
            await this.redis.del(`workspaces:user:${member.userId}`);
        }
    ...
    async inviteMember(
        ...
        // Invalidate cache
        await this.redis.del(`workspaces:user:${userToInvite.id}`);
    ...
    async removeMember(workspaceId: string, removerId: string, memberUserId: string) {
        ...
        await this.redis.del(`workspaces:user:${memberUserId}`);
