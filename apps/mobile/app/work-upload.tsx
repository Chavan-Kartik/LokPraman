...
export default function WorkUploadScreen() {
    ...
    const uploadImages = async (retryCount = 0) => {
        ...
        try {
            ...
            const response = await tasksAPI.submitWork(taskId, formData);
            console.log('[WorkUpload] Upload successful:', response);

            // Access ai verification results here if needed
            const aiVerdict = response.aiResult?.verdict || response.aiVerification?.verdict;
            const score = response.aiResult?.score || response.aiVerification?.score;

            Alert.alert(
                aiVerdict === 'FIXED' ? 'AI Verification Passed!' : 'Upload Successful',
                `Your images have been submitted. ${aiVerdict ? `AI Status: ${aiVerdict} (${score}% match).` : 'The supervisor will review.'}`,
                [
                    {
                        text: 'OK',
