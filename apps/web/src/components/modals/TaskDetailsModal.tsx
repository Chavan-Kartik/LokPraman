...
export const TaskDetailsModal = ({ taskId, isOpen, onClose, onTaskUpdated }: TaskDetailsModalProps) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div
                className="w-full max-w-4xl rounded-sm shadow-2xl my-4 max-h-[90vh] flex flex-col bg-[var(--background-elevated)]"
            >
                {/* Header */}
                <div className="flex items-center justify-between p-4 sm:p-6 border-b border-[var(--border-default)] flex-shrink-0 bg-[var(--background-elevated)]">
                    <h2 className="text-lg sm:text-xl font-bold text-[var(--foreground)]">
                        Task Details
                    </h2>
                    <button
                        onClick={onClose}
                        className="p-2 rounded-full transition-colors flex-shrink-0 hover:bg-[var(--surface)] text-[var(--foreground-muted)]"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-4 sm:p-6 overflow-y-auto flex-1">
                    {loading ? (
                        <div className="space-y-4 animate-pulse">
                            ...
                            <div className="w-full h-48 sm:h-64 rounded-lg bg-[var(--surface-hover)]" />

                            {/* Title skeleton */}
                            <div className="h-8 w-3/4 rounded bg-[var(--surface-hover)]" />

                            {/* Badges skeleton */}
                            <div className="flex gap-2">
                                <div className="h-6 w-20 rounded bg-[var(--surface-hover)]" />
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-4">
                                <div className="h-16 rounded bg-[var(--surface-hover)]" />
                                <div className="h-16 rounded bg-[var(--surface-hover)]" />
                            </div>
                        </div>
                    ) : !task ? (
                        <div className="flex justify-center items-center h-64 text-[var(--foreground-muted)]">
                            Task not found
                        </div>
                    ) : (
                        <div className="space-y-5">
                            {/* Image Carousel */}
                            <div className="relative w-full h-48 sm:h-64 md:h-72 max-h-[40vh] rounded-lg overflow-hidden bg-[var(--surface)]">
                                        ...
                                        {task.beforeImages.length > 1 && (
                                            <>
                                                <button
                                                    ...
                                                    className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full transition-all bg-[var(--background-elevated)]/85 hover:bg-[var(--background-elevated)] text-[var(--foreground)]"
                                                >
                                                    <ChevronLeft size={20} />
                                                </button>
                                                <button
                                                    onClick={() => setCurrentImageIndex(prev => prev === task.beforeImages.length - 1 ? 0 : prev + 1)}
                                                    className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full transition-all bg-[var(--background-elevated)]/85 hover:bg-[var(--background-elevated)] text-[var(--foreground)]"
                                                >
                            ...
                            <div className="flex gap-3">
                                ...
                                {task.locationName && (
                                    <a
                                        ...
                                        className="flex items-center gap-2 p-3 rounded-lg transition-colors flex-1 min-w-0 bg-[var(--surface)] hover:bg-[var(--surface-hover)]"
                                    >
                                        <img src="/map.svg" alt="Location" className="w-5 h-5 flex-shrink-0" />
                                        <p className="text-sm font-medium truncate text-[var(--foreground)]">
                                            {task.locationName}
                                        </p>
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-auto flex-shrink-0 text-[var(--foreground-muted)]">
