...
const Calendar = () => {
    ...
    return (
        <DashboardLayout>
            <div className="h-full flex relative overflow-hidden linear-reveal">
                {/* Main Calendar Area */}
                <div className={`flex-1 min-w-0 p-6 flex flex-col transition-all duration-300`}>
                    ...
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex rounded-lg overflow-hidden border border-[var(--border-default)] bg-[var(--background-elevated)]">
                                <button
                    ...
                    {/* Calendar Card */}
                    <div className="flex-1 rounded-2xl linear-elevated p-4 overflow-auto">
                        {/* Navigation */}
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-4">
                                <button
                                    onClick={goToPreviousMonth}
                                    className="p-2 rounded-lg transition-colors text-[var(--foreground-muted)] hover:text-[var(--foreground)] hover:bg-[var(--surface)]"
                                >
                                    <ChevronLeft size={20} />
                                </button>
                            ...
                            <button
                                onClick={goToToday}
                                className="px-4 py-2 rounded-lg text-sm font-medium transition-colors bg-[var(--surface)] text-[var(--foreground)] hover:bg-[var(--surface-hover)]"
                            >
                                Today
                            </button>
                        </div>
                        ...
                        <div className="grid grid-cols-7 gap-1 mb-2">
                            {dayNames.map(day => (
                                <div
                                    key={day}
                                    className="text-center py-2 text-sm font-medium text-[var(--foreground-muted)]"
                                >
                                    {day}
                                </div>
                            ))}
                                ...
                                return (
                                    <button
                                        ...
                                        className={`h-24 p-2 rounded-lg border transition-all text-left flex flex-col ${isSelected(date)
                                            ? 'border-[var(--border-accent)] bg-[color:rgba(94,106,210,0.14)]'
                                            : isToday(date)
                ...
                >
                    ...
                    <button
                        ...
                        className="absolute -left-5 top-1/2 -translate-y-1/2 z-20 w-10 h-20 flex items-center justify-center rounded-l-xl bg-[var(--accent)] hover:bg-[var(--accent-bright)] text-white shadow-[0_8px_26px_rgba(94,106,210,0.4)] transition-all hover:scale-105"
                    >
                    ...
                    >
                        <div className="h-full border-l border-[var(--border-default)] bg-[var(--background-elevated)]/70 p-4 overflow-auto backdrop-blur-md">
                            {/* Panel Header */}
                            ...
                            {!selectedDate ? (
                                ...
                                <div className="space-y-3">
                                    {selectedDateTasks.map(task => (
                                        <div
                                            key={task.id}
                                            className="p-4 rounded-xl border border-[var(--border-default)] bg-[var(--surface)] transition-all duration-200 hover:border-[var(--border-hover)] hover:bg-[var(--surface-hover)] cursor-pointer"
                                        >
                                            ...
                                            <h4 className="font-semibold mb-1 text-[var(--foreground)] tracking-tight">
                                                {task.title}
                                            </h4>
                                            <p className="text-sm line-clamp-2 mb-3 text-[var(--foreground-muted)]">
                                                {task.description}
                                            </p>
                                            <div className="flex items-center justify-between gap-2">
                                                ...
                                                {userRole === 'CLIENT' ? (
                                                    ...
                                                    <div className="flex items-center gap-1.5">
                                                        <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-[var(--accent)] text-white">
                                            ...
                                            {task.locationName && (
                                                <div className="mt-2 pt-2 border-t border-[var(--border-default)] flex items-center gap-1 text-xs text-[var(--foreground-muted)]">
                                                    <MapPin size={12} />
